...
public class WizardComputerPlayer2 extends GameComputerPlayer {
    //Tag for logging
    private static final String TAG = "WizardComputerPlayer2";
    /**
     ...
     * 		the player's name
     */
    public WizardComputerPlayer2(String name) {
    ...
    protected void receiveInfo(GameInfo info) {
        ...
        // if it's not a WizardState message, ignore it; otherwise
        // cast it
        if (!(info instanceof WizardState)) return;
        WizardState myState = (WizardState)info;
        ...
        if (win != null) {
            Logger.log("WizardComputer", "sending action");
            game.sendAction(new WizardMoveAction(this, win.y, win.x));
            return;
        }
        ...
        if (loss != null) {
            Logger.log("WizardComputer", "sending action");
            game.sendAction(new WizardMoveAction(this, loss.y, loss.x));
            return;
        }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (myState.getPiece(j, i) == ' ') {
                    if (selectCount == 0) {
                        // make the move
                        game.sendAction(new WizardMoveAction(this, j, i));
                        return;
                    }
                    selectCount--;
                }
     ...
     */
    private Point findWin(WizardState state, char thePiece) {
    ...
    private Point helpFindWin(WizardState state, char thePiece, int rowStart,
